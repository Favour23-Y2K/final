...
if ($isChatUpload) {
    $mediaServerUploadDir = $type === 'image' ? '/photo/direct/' : 
                            ($type === 'video' ? '/video/direct/' : 
                            ($type === 'voicenote' ? '/voicenotes/' : '/attachment/'));
} else {
...
if (!empty($_FILES['media'])) {
    ...
    // Generate the thumbnail name using the unique filename
    $thumbnailName = md5($uniqueName) . '.png'; // This line can be removed if you want to use the unique name directly
    // If you want to use the unique name directly, uncomment the following line
    // $thumbnailName = $uniqueName; // Use the unique name for the thumbnail
